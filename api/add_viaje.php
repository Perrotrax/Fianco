...
if (!$destino) {
    echo json_encode(['success' => false, 'message' => 'El destino es obligatorio']);
    exit;
}
...
if ($stmt) {
    ...
    if ($stmt->execute()) {
        echo json_encode(['success' => true, 'message' => 'Viaje creado con éxito']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Error al guardar el viaje: ' . $stmt->error]);
    }
    $stmt->close();
} else {
